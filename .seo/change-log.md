...
- Security and permissions: values are rendered only with `createElement`, `textContent`, `append` and `replaceChildren`. Anonymous access must be Viewer only; the Sheet must contain only public price data and no patient data, secrets or internal medical information.
...
- Remaining blocker: the owner must perform the one-time Google permission action `Anyone with the link → Viewer`. After that, verify the public feed and rerun the full repository, SEO, diff and 360/768/1440 rendered checks before any merge.
