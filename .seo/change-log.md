...
- URLs/templates changed: `/price.html`, new `/price-loader.js`, setup documentation, price-feed tests, the shared static validator and the minimum matching `.seo/` controls. No CSS, sitemap, robots, title, description, canonical, JSON-LD, navigation, footer, contact or other-page change was made.
- Initial implementation: the first draft used `Google Sheets → Apps Script Web App → JSON → price-loader.js → price.html`, with a manual Apps Script deployment blocker.
- Simplification: the owner requested a no-Apps-Script flow. The branch now uses `Google Sheets → Google Visualization read-only feed → price-loader.js → price.html`. The fixed Spreadsheet ID and seven sheet names are allowlisted in the loader; Apps Script source and tests were removed.
- Loader behavior: each tab is queried through a fixed Google endpoint with a 7-second timeout and cache busting. Blank/incomplete editing rows are ignored, values are validated, markup/script-like text is rejected, all seven category results must succeed, and no visible list is replaced until the complete payload and DOM targets pass.
- Fallback and price integrity: the complete current HTML price remains immediately visible and unchanged on access, timeout, network, response, category, service or validation failure. The separate orthodontic note no longer duplicates the numeric price, so a future Sheet change cannot leave a stale second value; the confirmed static `17 000 грн` source row remains.
- Security and permissions: values are rendered only with `createElement`, `textContent`, `append` and `replaceChildren`. The site technically requires only public read access; the Sheet must contain only public price data and no patient data, secrets or internal medical information.
- Automated validation: 17 Node tests passed for fixed-tab loading, URL construction, names, prices, add/delete/reorder, blank/incomplete rows, generated display IDs, empty categories, malformed responses, schema/category checks, long and markup-like values, numeric cells, XSS-safe DOM construction and atomic fallback preservation. JavaScript syntax passed locally for the revised loader.
- Access state at initial validation: public feed verification and the full repository, SEO, diff and 360/768/1440 rendered checks remained pending before any merge.
- Commit or pull request: follow-up commits on `feat/google-sheets-price`; [draft PR #7](https://github.com/optidigitalagent/status1/pull/7). Merge and deployment intentionally not performed.

## 2026-07-29 — Owner-approved public editing and live Visualization verification

- Owner decision: the owner intentionally selected `Anyone with the link → Editor`, confirmed that anyone receiving the link can change the Sheet, and accepted that risk. Google Drive permissions were not changed by code.
- Technical requirement: the website needs only anonymous public reading. Viewer is an optional safer alternative, not a requirement or blocker.
- Live read-only evidence: all seven fixed category tabs returned HTTP `200`, `application/javascript; charset=utf-8`, Google Visualization `status: ok`, and columns A `ID`, B `Назва послуги`, C `Ціна` without authentication. The `Інструкція` tab was not requested.
- Data evidence: 26 completed services were read across the seven tabs (2/3/3/4/9/3/2). Column C remained textual in every completed row, including the exact current orthodontic value `17 000 грн`.
- Scope: direct architecture remains `Google Sheets → Google Visualization feed → price-loader.js → price.html`; Apps Script was not restored and no Sheet data or permissions were changed.
- Validation: the live loader completed all seven public JSONP queries and atomically rendered 2/3/3/4/9/3/2 rows with the exact `17 000 грн` value. The 17 price-loader tests, JavaScript syntax, SEO/HTML/JSON-LD/canonical/H1/link/fragment/local-resource validation, conversion contract and `git diff --check` passed. Static fallback counts remained 2/3/3/4/9/3/2, and both phones, address and Instagram remained in `price.html`.
- Render limitation: this session's browser runtime exposed no available browser, so a fresh 360/768/1440 rendered check, horizontal-overflow check and browser-console check could not be repeated. The code and public HTML/CSS were not changed in this documentation-only follow-up; the prior PR viewport evidence remains the latest browser evidence.
- Remaining gate: merge and deployment still require separate explicit owner authorization. A fresh rendered browser pass remains recommended when a browser runtime is available, but public Sheet access is not a technical blocker.
